...
@observer
export default class HeldList extends Component {
    constructor(props) {
        super(props);
    }
    ...
    updateList = () => {
        ...
        myClient.submitWithToken('GET', '/protected/held/current')
            .then(
                action((response) => {
                            ...
                            current.map(e => {
                                if (e.connectionId !== conn.connectionId) {
                                    return <ListGroupItem className='p-1 m-1'
                                                          key={e.connectionId}>{e.connectionId} ({e.username})

                                        <ConfirmModal body='This will clear all held resources by this connection id. Coordinate with other user(s) first. '
                                                      header='Clear held'
                                                      uiElement={ <Octicon name='trashcan'
